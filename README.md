...
​       因1s=1000000000纳秒，故以上日志耗时满足在一秒内，故在每秒1000内应选择加锁的协程进行并发输出，以便查看，而每秒大于1000应选择无锁的版本。在以上基础上固定输出信息不变，调整每秒输出N的大小，每10分钟统计一次，记录日志收集状况。在测试过程中发现当日志输出>1500时，在程序前开始可能会造成时间间隔>1s,为2s的情况，故需等待几秒开始记录。
...
为了进一步测试，将输出信息减少，改为输出单行，对比来查看flunet-bit情况，日志打印信息如下
...
对于多个Pod测试，在本次测试中，我们使用Deployment创建2个pod，控制pod中每个容器的打印速率，观察日志打印状态
...
通过调整输出，第一次将输出信息一次发送一条，1秒钟发送1000条，1秒钟发送1500条的情况下，查看elasticsearch-logging-data-0和1中es容器的资源使用情况，发现1s发送1000条资源使用量为0.29core，而1秒发送1500条情况下资源使用量为0.29-0.32core，内存消耗为2.29G，且es容器资源限制为CPU：4，初步排除es性能不够的影响，通过调整fluent-bit的CPU来查看信息
...
| ----------------- | ---------------- | ------------------------ | ----------------------- | ------------------ | ----------------------- | ----------------------------- | ------------------------------ | --------------------------------------- |
| 每秒输出800次日志 | 802              | 481147                   | 403001                  |                    |                         | 0.1-0.11core                  | 24-30                          | 17.99Mbps/2.58Mbps                      |

修改Input.spec.tail.memBufLimit,将其默认值5MB改为20MB    30MB

N=2



|                                         | 每秒输出800次日志  | 每秒输出1000次日志  | 每秒输出1500次日志       | 每秒输出2000次日志       |
| --------------------------------------- | ------------------ | ------------------- | ------------------------ | ------------------------ |
| 测试程序实际打印                        | 802                | 1002                | 802                      | 2002                     |
| 10分钟内共有（容器内部）                | 477190             | 602202              | 901200                   | 1201200                  |
| 10分钟内共有（fluentd）                 | 473539             | 602589              | 900747                   | 1154136                  |
| 平均每秒收集(容器)                      | 795.316            |                     |                          | 2002                     |
| 平均每秒收集（fluentd）                 | 795.3              |                     |                          | 1923.56                  |
| fluent-bit资源使用情况（CPU）           | 0.11-0.12core      | 0.14-0.15core       | 0.21-0.22core            | 0.26-0.29core            |
| fluent-bit资源使用情况（内存）          | 21.8-31.37Mi       | 51.84-55.71Mi       | 46.01-63.54              | 58.11-88.66              |
| fluent-bit资源使用情况（网络流出/流入） | 20.21Mbps/2.87Mbps | 24.09Mbps/3.59MKbps | 35.55Mbps/5.5Mbps        | 41.67-46.68Mbps/7.09Mbps |
| fluentd资源使用情况（CPU）              | 72-77m             | 87-88m              | 0.14-0.15core            | 0.16-0.17core            |
| fluentd资源使用情况（内存）             | 139.28Mi           | 154.93Mi            | 212.39Mi                 | 265.48Mi                 |
| fluentd资源使用情况（网络流出/流入）    | 8.05Kbps/8.96Kbps  | 10.72Kbps/10.32Mbps | 20.7Kbps/15.94-17.87Mbps | 25.23kbps/20.56Mbps      |